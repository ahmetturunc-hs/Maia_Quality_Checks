...
You see ONLY the prompt text, which responses were flagged as failures, and the failure reasoning written for Response 2. You do **not** see any attached files, input documents, model responses, or other artifacts. **Treat every file mentioned by name as legitimately attached.** Do not flag the submission for missing attachments, missing input files, or "unverifiable" file references.

You also cannot verify whether the failure reasoning is **factually accurate** about Response 2 (since you can't see the response). Your job is only to evaluate whether the reasoning text is **specific, concrete, and severe** — not whether each claim is true. If a quality area would only fail because you cannot read a referenced file or response, do NOT fail it.
...
- Do not fact-check the reasoning's claims against the golden answers. You still cannot see Response 2, so you cannot verify whether the claimed defect is actually present — your job is still to evaluate whether the reasoning is specific, concrete, and severe.
...
1. `1` is NOT in the `failing_models` array — Response 2 was not marked as a failure, so there is no failure reasoning to evaluate.
...
```json
{
  ...
  "verdict": "passed",
  "feedback": "Response 2 not marked as failed — no failure reasoning to review.\n\nThis linter can over-flag. If something seems off, feel free to ignore."
}
```
...
A passing failure reason should read like a senior reviewer pointing at concrete defects in Response 2. The reasoning should:
...
> 1. Response 2 overstates the arbitrator's remedial authority by suggesting he/she can impose "enhanced damages and attorney's fees" under the Back Pay Act, which in reality authorizes make-whole back pay and related monetary relief, not punitive damages.
>
> 2. Response 2 treats any non-compliance during the retained-jurisdiction period as "itself a separate ULP," confusing it with refusing a final award and interim implementation disputes.
>
> 3. Response 2 incorrectly implies the union can both bypass a new case and also "petition the FLRA under 5 U.S.C. § 7122(b)."
...
> 1. Response 2 refused to draft the memorandum, stating it could not provide legal advice. The prompt explicitly framed this as a General Counsel briefing for the Secretary and required a 700-word legal memo with bluebook citations addressing six enumerated sub-questions. The deliverable does not exist.
...
> 1. Response 2 calculates Q3 revenue as $4.2M when the source spreadsheet shows $5.8M, a 27% understatement that propagates through the entire financial section.
>
...
> 1. Response 2 misspells "received" as "recieved" in section 2.
...
```
Response 2 not marked as failed — no failure reasoning to review.
...
- **Auto-pass** → `"Response 2 not marked as failed — no failure reasoning to review."`
